               ...
               if ( type === 'display' ) {
                   var bt = '<span id="botoes_id'+data+'">'; 
                    ...
                    if($("#all_permissions").val().indexOf('paciente-editar') > -1){
                        bt += '<a data-toggle="modal" data-target="#modalDefault" href="#" idEdit="'+data+'" class="editar marginIcon"><i title="Editar consulta" data-toggle="tooltip" data-placement="bottom" class="fas fa-edit fa-lg"></i></a>';
                    }
...
function verificaAgenda(dataSelecionada, dataAtual, agendaHorarios, agendaDados){
    ...
    if(agendaHorarios.length != 0){
        $(".agenda-todos").css({'color':'black','text-decoration':'none','font-weight':'normal'}).removeClass( "pisca_pisca" );
        $(".editar,.apagar").show();
        $.each(agendaHorarios, function() {//console.log(moment().format('DD/MM/YYYY H:mm')+' - '+moment().format('YYYY-MM-DD '+$(this).html()));
            ...
            if($(this).attr('status') == 'Chamado'){
                $(".agenda-dados"+$(this).attr('id')).css('font-weight', 'bold').addClass( "pisca_pisca" );
            }
...
channel.bind('App\\Events\\AgendaStatusEvento', function(data) {
    if(moment().format('DD/MM/YYYY') == data.agenda.data && $("#unidade").val() == data.agenda.unidade_id){
        ...
        if(data.medicoId == $("#user_id").val()){
            ...
            if($('#user_type').val() == 'Medico' && data.agenda.status_nome == 'Em atendimento'){ 
...
$("#unidade").on("change", function(){
    getAgendamentosUnidade();
});

getAgendamentosUnidade();

function getAgendamentosUnidade(){
    $.get( $("#rota-agendamentos").val()+'/'+$("#unidade").val(), function( res ) {
