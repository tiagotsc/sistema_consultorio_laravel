...
<div class="col-md-12">
    ...
    <hr>
    {!! Form::model($dados, ['id' => 'frm', 'method' => 'put', 'route' => ['relatorios.update', 'id'=>$dados->id]]) !!}
    <div class="row">
        <div class="form-group col-md-2 semPadding">
        {!! Form::label('relatorio_categoria_id', 'Categoria') !!}<span class="obrigatorio">*</span>
        {!! Form::select('relatorio_categoria_id', $categorias, old('relatorio_categoria_id'), ['class' => 'form-control', 'readonly' => false]) !!}
        </div>
        ...
        {!! Form::text('descricao', null, ['class' => 'form-control', 'maxlength' => '255', 'placeholder' => 'Preencha...']) !!}
        </div>
        <div class="form-group col-md-2 semPadding">
        {!! Form::label('banco_conexao', 'Banco conexão') !!}<span class="obrigatorio">*</span>
        {!! Form::select('banco_conexao', $bancosDeDados, old('banco_conexao'), ['class' => 'form-control', 'readonly' => false]) !!}
        </div>
        <div class="form-group col-md-2 semPadding">
    ...
    <div class="row">
        <div class="col-md-8 divOverFlowPequena">
            <h5>Lista de campos para preenchimento 
            ...
            <table id="tabelaRelatorioCampos" class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="tableSemPadding">
                            <label class="labelSemNegrito">
                                {{ Form::checkbox('campo['.$cont.']', $campo->id, true, array('class' => 'marcado','identcampo'=>$cont)) }} 
                            </label>
                        </td>
                        <td class="tableSemPadding"><a data-toggle="tooltip" data-placement="bottom" title="{{ $campo->legenda }}" href="#">{{ $campo->nome }}</a></td>
                        <td class="tableSemPadding form-group">{!! Form::text('campo_nome['.$cont.']', $campo->pivot->nome, ['class' => 'form-control campos campo_nome campo_marcado','cont' => $cont, 'maxlength' => '255', 'placeholder' => 'Preencha...']) !!}</td>
                        <td class="tableSemPadding form-group">{!! Form::text('campo_legenda['.$cont.']', $campo->pivot->legenda, ['class' => 'form-control campos campo_marcado campo_legenda'.$cont, 'maxlength' => '255', 'placeholder' => 'Preencha...']) !!}</td>
                        <td class="tableSemPadding">{!! Form::select('campo_obrigatorio['.$cont.']', array('S'=>'Sim','N'=>'Não'), $campo->pivot->obrigatorio, ['class' => 'form-control campos campo_marcado_obrigatorio campo_obrigatorio'.$cont, 'readonly' => false]) !!}</td>
                        <td class="tableSemPadding">{!! Form::text('campo_ordem['.$cont.']', $campo->pivot->ordem, ['class' => 'form-control campos campo_marcado_ordem campo_ordem'.$cont, 'maxlength' => '1']) !!}</td>
                    </tr>
                        @php $cont++; @endphp
                    @endforeach
                    ...
                    <tr>
                        ...
                        <td class="tableSemPadding">{!! Form::text('campo_ordem['.$cont.']', '', ['class' => 'form-control campos campo_ordem'.$cont, 'maxlength' => '1']) !!}</td>
                    </tr>
                        @php $cont++; @endphp
                    @endforeach
                </tbody>
            ...
            <div id="campos_sumarizacao">
                <table id="tabelaRelatorioCamposSumarizados" class="table">
                    ...
                    <tbody id="sumarizacao_adicionada">
                        @php $cont = 0; @endphp
                        @foreach($dados->sumarizacao as $suma)
                        <tr>
                            <td class="tableSemPadding form-group">
                                <input type="text" class="form-control" name="campo_observado[{{$cont}}]" value="{{$suma->campo}}">
                            </td>
                            <td class="tableSemPadding form-group">
                                <input type="text" class="form-control" name="campos_sumarizados[{{$cont}}]" value="{{$suma->sumarizados}}">
                            </td>
                            <td class="tableSemPadding align-middle" width="7px">
                                <a href="#" data-toggle="tooltip" data-placement="bottom" title="Remover" class="remover_sumarizacao"><i class="fas fa-minus-circle fa-lg"></i></a>
                            </td>
                        </tr>
                        @php $cont++; @endphp
                        @endforeach
    ...
    <div class="row">
                ...
                @foreach($perfis as $value)
                    <li class="list-group-item">
                        <label class="labelSemNegrito" data-toggle="tooltip" data-placement="bottom" title="{{$value->label}}">
                            {{ Form::checkbox('perfis[]', $value->id, in_array($value->id, $perfisMarcados)? true: false, array('class' => 'perfil_checkbox '.$value->grupo.'_checkbox')) }} 
                            {{ $value->name }}
                        </label>
                    </li>
                @endforeach
        ...
        <div class="col-md-8 form-group">
                ...
                {!! Form::textarea('query', old('query'), ['class' => 'form-control', 'rows'=>30, 'placeholder' => 'Cole a query aqui...']) !!}
        </div>
    </div>
    <div class="row">
